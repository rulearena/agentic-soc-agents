...
（triage-l2-soc-analyst 目前無 active backlog；已 resolved 項見底部 Changelog）
...
- 2026-06-12: `TUN-L2-005` resolved in this PR — added `Break-glass Parallel Path（L1 已直接升 IR）` sub-section to `triage-l2-soc-analyst.md` §工作流程（Step 5 後）+ `### 5. L2 → IR Initial Sync（Break-glass 並行路徑）` template to §溝通範本；補充 L1 break-glass 直接升 IR 後 L2 的並行操作路徑：最小 containment set（Approved Playbooks 範圍內，觸發條件仍獨立成立不因並行放寬）、與 IR 的同步節奏（IR 指定頻率、Initial Sync 盡快送出不等 investigation 全完）、三段式分段 handoff（Milestone A/B/C、每欄 confirmed/pending/unknown 標示）；邊界段明示 L1 break-glass 不取代 L2 的 escalation report 義務。v1.3 low-sensitivity review lane. P2.
