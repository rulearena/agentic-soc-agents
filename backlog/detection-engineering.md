...
（detection-engineering-threat-hunter 目前無 active backlog；已 resolved 項見底部 Changelog）
...
- 2026-06-12: `TUN-HUNT-005` resolved in this PR — added `### Response to TI Attribution Request` template to `detection-engineering-threat-hunter.md` §溝通範本（緊接既有 `Hunt finding handoff to Threat Intel` 後）；當 TI Analyst 以外部 actor report 對應 Hunt finding 並邀請 Hunter 在 HFP 加入 attribution 句時的拒絕範本：三層拒絕理由（角色分工·HFP 記技術 observable attribution 屬 TI / 方法學邊界·相同 tooling·infrastructure 可被多方共用或模仿超出 HFP 可下結論範圍 / attribution ownership·結論出現在誰的 output 後續 IR·Legal 引用責任就在誰）+ 替代協作（HFP observed facts 保留供 TI 做 actor profile 評估·需補技術細節從 HMD 補·TI 自產 actor profile 引用 HFP 作 technical input）；attribution 結論屬 TI 的輸出不屬 HFP. Attribution family member（cross-ref joint framing：source label ≠ analyst conclusion·raw metadata ≠ distribution output·attribution ownership 跟著 output 走）；反模式 #6「在 finding 中下 attribution 結論」已涵蓋一般案例，本範本補被 TI 邀請時的具體回應路徑. v1.3 medium-sensitivity review lane.
