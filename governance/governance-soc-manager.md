...
### 6. Individual Development Review — Separation Statement

PIR(Post-Incident Review)與個人發展檢討(individual development review)是**兩份永不互相引用、永不合併**的文件。PIR 全程 gap framing(systemic / process / training / coverage / tooling)、owner-by-role、**不點名個人**(紅線 B);個人能力 / 績效議題若需處理,另開獨立對話,**走 role owner + HR 管道,不寫進任何 PIR artifact**。本範本固化兩者的分流聲明與各自的證據基礎。

```markdown
# Individual Development Review — Separation Statement — IDR-[period]

**目的:** 聲明本次 individual development review 與 PIR / SOC metrics **完全分流**,避免 incident 事後檢討被誤用為個人究責。

## 文件分流(Document Separation)
- **PIR Report** = lessons-learned facilitation,gap framing,owner-by-role,**不含個人姓名、不含個人績效判斷**
- **Individual Development Review** = 個別成員的能力發展對話,**不在 PIR、不在 SOC Metrics & Trend Report 內進行**
- 兩份文件永不互相引用、永不合併;PIR 的 action item 不得被改寫成某個人的績效缺失

## 歸屬與管道(Ownership & Channel)
- **主導:** 該成員的 role owner(line manager),**非 SOC Manager 以 PIR facilitator 身份**
- **協同:** HR(發展計畫、職涯路徑、訓練資源)
- SOC Manager 在此**僅提供制度性 context**(團隊整體 skill gap、訓練資源規劃),**不對個別成員下績效結論**

## 證據基礎(Evidence Basis)
- 採 **qualitative review + skill matrix**(能力面向、訓練完成度、職涯目標對齊)
- **不採 SOC metrics**(MTTD / MTTC / alert volume / closure rate 等)作為個人績效指標——metrics 僅用於 systemic improvement(見 §SOC Metrics & Trend Report 的 Note on Metrics Usage)
- 個案 incident 中的個人表現**不被抽出**作為發展檢討素材;若有系統性訓練缺口,以 PIR 的 training gap(角色層級)回饋

## 邊界(Boundaries)
- 觀察到「某 incident 中某成員處理不當」→ 屬 role owner + HR 的 development 對話,**不在 PIR 點名**、不寫進 governance review 紀錄的個人欄
- gap framing 與個人發展對話的界線:systemic / process / training / coverage / tooling gap 入 PIR(角色層級);個人能力成長入 dev review(成員層級),兩者各走各的文件與管道
```
