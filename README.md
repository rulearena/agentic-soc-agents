...
這些角色定義檔本身**不含任何真實資料**——它們是 prompt / 工作流參考。實務上你會拿它們對**自己環境的真實 log**（SIEM / EDR 等）運行，這是預期用法，**無需脫敏**。

唯一需要脫敏的時機是**對外公開**：當你把某次運行的產物（finding package、hunt 報告、demo 截圖等）放進 public repo、影片、blog、社群或任何給觀眾看的內容時，先移除可辨識的組織資訊（domain、主機名、IP、廠牌、防禦盲區等）再公開。

> 「私下對真實資料運行」與「公開展示品」是兩條軌道：前者照常，後者先脫敏。
