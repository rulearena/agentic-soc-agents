...
> **candidate / parking lot 不是 commitment**：backlog 標為 v1.2 candidate 的條目是候選池，不承諾納入任何特定 release。
...
> **v1.1 / v1.2 狀態**：下列 4 themes 的 representative P1 controls 已 shipped（故標 🔵 in progress）。各 theme 的 remaining 條目（P2 / attribution family / cross-role 整合）為 **candidate，非 commitment**——🔵 表示「代表性控制已進 main」，不表示整個 theme 已完工。完整 tactical 狀態見 [`backlog/triage-v1.1.md`](backlog/triage-v1.1.md)。v1.2 再併入一組 P2 boundary / scope / cadence 細化（落在上述 themes 內），狀態維持 🔵（代表性控制已進 main、theme 未完工的判定不變）。

### 1. Authority Pressure Handling 🔵 in progress
...
### 2. Cross-Role Refusal Templates 🔵 in progress
...
### 3. Operational Boundary Specification 🔵 in progress
...
### 4. Public-Facing Wording & Disclaimers 🔵 in progress
