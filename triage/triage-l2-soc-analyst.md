...
### 跨單位同一 artifact：hunt 與 IR 並行起步

§升級條件 的跨單位 artifact 硬規則命中、自動升 IR 時，L2 **不是**只把球交給 IR、等 IR 回頭再決定要不要 hunt。跨單位同一 artifact 是強烈的跨環境風險信號，hunt 與 IR 是**兩條並行路徑**：
- **升 IR**：依 §升級條件 走，trigger 條件以該章節為準，這裡不重述。
- **同步建立 hunt seed**：把跨單位 artifact 當 hunt hypothesis backlog item 記錄下來（沿用上表 Threat Hunter 列的既有通道），讓 Threat Hunter 可平行評估，不等 IR 反向通知才補。

兩條路徑並行起步，不是「先升 IR、hunt seed 等 IR 回頭才補」。hunt 屬平行協作、不是升級鏈；artifact 歸因與 incident 定性仍由 IR / Threat Hunter 各自負責，L2 不自下 supply chain 結論。
