...
### IR-A 跨 rotation 接班簡報 (IR-A Shift Handoff Brief)

事件持續超過一個 rotation（通常 > 4 小時）時，下班的 IR Analyst（rotation A）對接班者（rotation B）的執行狀態交接簡報。**這是同角色跨班交接，不是 §「對 L2 verification owner 的 handoff」**——後者是把單一 action 的 verification monitor 移交給 L2；本簡報交接的是 IR-A 自己手上「未結的執行 / 等待 / holding」整體狀態。所有引用到的機制（Scope Drift Report、verification monitor）沿用既有定義，本簡報只列當下狀態快照，**不重述流程**。

```
[Shift Handoff Brief] INC-2026-0042 / IR-A rotation A → rotation B
接班時間: 18:00

== AAR 狀態 ==
- Executing: AAR-009 isolate host 1–4 of 6（執行到第 4 台，未完成）
- Completed: AAR-003、AAR-007（已 verify、Execution Report 已歸檔）
- Pending IRC approval: AAR-011（process kill，等 Forensics preservation 確認後 IRC 核准）

== Holding scope drift items ==
- SDR-002 已提交 IRC，holding endpoint 9–12 unexecuted，等 IRC 對擴大 scope 的決策（細節見 SDR-002，本處不重述）

== Verification monitor 移交 ==
- AAR-003：verification monitor 已於 14:39 轉 L2（見 §對 L2 verification owner 的 handoff），rotation B 無需接手
- AAR-007：verification monitor 仍由 IR-A 持有 → 本次一併移交 rotation B

== 未結 Execution Report ==
- AAR-009 Execution Report 草稿未 finalize，預計 finalize ETA: 22:00（接班者續寫或待我下個 rotation 回來補，請在簡報確認由誰收尾）
```

### 對 IR Commander 的 Pending Action 待命回報（Pending Action Status Ping）

action 被 IRC BLOCK、或卡在前置條件（等 Forensics preservation、等其他執行者 / 系統）而進入 holding 時，IR Analyst **主動**按節奏回報自身待命狀態、確認 hold 是否仍成立，而非被動等到有人想起來才問——避免「卡住但沒人知道」在跨班次 / 跨人員時漏接。節奏依 IRC 指定 cadence、action urgency、blocker ETA / 狀態變更與事件 severity 調整，**不在此寫死固定時數**。本 ping 只回報自身狀態並把問題交回 IRC：**不自行解除 BLOCK、不替 IRC 決定是否轉手執行者、不自行變更或重新指派他人手上的 pending action**——是否轉手或調整優先序由 IRC 協調決定（向 blocker owner 如 Forensics 詢問 ETA 屬正常協作，不在此限）。跨班次的整體狀態快照見 §IR-A 跨 rotation 接班簡報，本 ping 是班內主動節奏，不重述。

```
[Pending Status] INC-2026-0042 / AAR-011
Action: process kill × 3 endpoints（in-memory C2 implant）
Held since: 14:30，原因：等 Forensics preservation 確認後 IRC 核准
我方狀態: standby，preservation 一就位即可在核准後執行
請 IRC 確認: 此 hold 是否仍成立？是否需轉手其他執行者接續待命？
下次回報: 依約定 cadence，或 blocker 狀態變更時提前
```
