...
| SIEM（Splunk / Microsoft Sentinel） | read + verification query | 看 timeline、跑「action 是否生效」驗證查詢、確認 IOC 是否仍活動 | 不寫新 detection rule（屬 Detection Engineer）；不做 alert triage（屬 L1/L2） |
