...
### 7. Break-glass Post-incident Review Checklist（事後審查）

```markdown
# Break-glass Review — INC-2026-0042 / break-glass event #1

**Break-glass event:** L1 於 09:52 direct page IR Commander（未經標準逐級升級；L2 已同步通知補齊 investigation chain）
**Logged in Decision Log:** 是（原因已記於事件當下）
**Reviewed by:** SOC Manager / incident review board（post-incident，非事件當場）

| 審查維度 | 本次紀錄（事實） | 事後判讀（回顧） |
|---|---|---|
| Trigger context | 觸發當下的告警 / 資產情境（如短時間大量關聯告警、critical asset 受影響） | 與〈關鍵規則〉#7 的緊急情境定義是否相符 |
| Alternatives considered | 當下標準 L1→L2→IRC 路徑是否可行、為何沒走 | 標準路徑是否真的來不及，而非省事 |
| Evidence of urgency | 支持「等不及 L2」的客觀訊號（時序壓力、擴散速度、critical asset 暴露） | 緊急性是否有事實支撐、非主觀感受 |
| Investigation chain continuity | break-glass 後通知 L2 的時點、L2 接手補齊 chain 的情形 | 是否同步通知 L2、後續 investigation chain 是否完整銜接 |
| Approval trail | Decision Log 是否記錄原因與後續處置 | 紀錄是否完整、可稽核 |
| Rollback / follow-up needs | 是否暴露需校正的流程 / 訓練 / detection 缺口 | 移交哪個 owner（SOC Manager 流程 / Detection Engineer 偵測） |

## 判讀結論（post-incident，供 review board，非事件當場拍板）
- [ ] 合理使用 —— 上述維度站得住腳，無流程改進需求
- [ ] 合理但有 follow-up —— 使用正當，但暴露流程 / 偵測缺口，移交 owner 校正
- [ ] 需檢討 —— 緊急性不足 / 未補齊 chain / 紀錄不全，列入濫用模式 review
```

**填寫與判讀原則**

- 本檢核表只用於**事後**審查**已發生**的 break-glass direct page（見〈關鍵規則〉#7、反模式 #6），是給 post-incident review（SOC Manager / incident review board）的結構化輸入，服務於事後**問責、校正與流程改進**，不服務於事件當場決策。
- **不是授權工具**：本表**不**定義「符合哪些條件就可以 break-glass」、**不**新增任何可在事件當下援引、用來略過 L2 或其他關卡的權限。緊急通道是否成立仍依〈關鍵規則〉#7 的既有定義；本表只回答「這一次用得對不對、有沒有要改的地方」。
- 即使判讀為「合理使用」，也**不構成對未來情境的預先授權** —— 每次 break-glass 仍屬個案，仍須當下記入 Decision Log、仍須事後審查。
- 事後警訊（指向「需檢討」）：頻繁觸發、單一非關聯告警即 page、L1 純因「嫌 L2 慢」而非真實緊急 —— 這些是 review 的**回顧性警訊**，不是事前禁令清單。
- 判讀維度固定在 trigger context / alternatives considered / evidence of urgency / investigation chain continuity / approval trail / rollback·follow-up needs，**不**改寫成「哪些情況 commander 可直接跳過誰」。
