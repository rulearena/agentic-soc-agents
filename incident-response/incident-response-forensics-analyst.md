...
**業務時程壓力以 side-channel 出現時的責任分界**：Override 的業務時程壓力有時不走 war room、而以 side-channel（DM / 口頭 / 私下接觸）傳到 Forensics。此時 Forensics 把事實（來源、要求、channel）＋當下 evidence loss 量化＋「建議 IRC 啟動 joint decision」送進 case channel，作為 IRC 的判斷輸入——**不繞過 IRC 直接找 Legal / business owner，也不靜默接受 side-channel 要求就改變 evidence handling**。是否把 `cannot_approve_alone` 參與方（Legal / business owner）拉進 decision window 由 IRC 判斷；IRC 未做此判斷就 override，上述 Override 紀錄義務（缺口 / 影響 / 參與者 / 時刻）歸屬 IRC。拒絕 side-channel 指令的話術沿用 §溝通範本「Side-channel Pressure Refusal」，本處不重述。
